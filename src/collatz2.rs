    ...
    > {
        layouter.assign_region(
            ...
            |mut region| {
                ...
                let is_odd: Value<Assigned<F>> = entry - ((a.invert() * entry) * a);
...
impl<F: FieldExt> Circuit<F> for CollatzCircuit<F> {
    ...
    fn synthesize(
        ...
        let out_cell = chip.assign_last(layouter.namespace(|| "out"), nrows - 1, one)?;
        println!("out cell: {out_cell:?}");
